---
layout: post
title: "Ruby Tidbits: Cutting out the Block with an &"
...
So, what's really going on here? Well Ruby sees the '&' so it wants to convert the Proc to a block. Since we have a Symbol on the other side Ruby type converts it to a proc but calling to_proc on the Symbol which returns a Proc that takes one argument and calls the method named by the symbol on that object. The implementation of to_proc on the Symbol class might look something like this:
